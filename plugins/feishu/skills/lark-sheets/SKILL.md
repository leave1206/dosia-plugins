...
name: lark-sheets
version: 1.2.0
description: "飞书在线电子表格（云端协作表格）：在飞书云空间创建和操作在线表格。【触发边界】仅当用户明确要飞书/在线表格/云端协作/分享给同事，或操作对象已是飞书表格链接时使用；若用户只是要一份本地 Excel/CSV 文件（如『整理成 Excel』『导出给我』且未点名飞书），不要用本 skill —— 应写本地文件再用附件回传。功能：创建在线表格并写入表头数据、读写单元格、追加行、在已知表格查找、导出。若用户想按名称搜云空间表格，改用 lark-doc 的 docs +search 先定位资源。"
metadata:
  requires:
    bins: ["lark-cli"]
