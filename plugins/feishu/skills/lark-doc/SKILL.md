...
name: lark-doc
version: 1.1.0
description: "飞书云文档（云端协作文档）：在飞书创建和编辑在线文档。【触发边界】仅当用户明确要飞书文档/在线协作/分享给同事，或对象已是飞书文档/Wiki 链接时使用；用户要本地 Markdown/Word 文件（未点名飞书）则写本地 + 附件回传，不要用本 skill。功能：从 Markdown 建文档、读取/更新文档（追加/覆盖/替换/插入/删除）、上传下载文档内图片文件、搜索云空间文档。当用户已在飞书语境下需按名称定位飞书电子表格/报表等云对象时，可用本 skill 的 docs +search 做资源发现。"
metadata:
  requires:
    bins: ["lark-cli"]
...
## 快速决策
- **前提：用户已在飞书语境下**（明确说飞书 / 在飞书里 / 给的是飞书链接）。用户泛泛说“表格 / Excel / 报表”而**未点名飞书**时，不要默认走飞书 —— 很可能是要本地文件，应写本地 + 附件回传。
- 在飞书云空间里按名称找“一个飞书表格 / 飞书报表 / 最近打开的飞书文档”，先用 `lark-cli docs +search` 做资源发现。
- `docs +search` 不是只搜文档 / Wiki；结果里会直接返回 `SHEET` 等云空间对象。
- 拿到 spreadsheet URL / token 后，再切到 `lark-sheets` 做对象内部读取、筛选、写入等操作。

## 补充说明 
`docs +search` 除了搜索文档 / Wiki，也承担“在飞书语境下先定位云空间对象，再切回对应业务 skill 操作”的资源发现入口角色。**仅当用户已明确指向飞书**时从这里开始；用户只说“表格 / 报表”而没点名飞书，不构成走飞书的理由。
