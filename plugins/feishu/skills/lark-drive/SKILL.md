...
name: lark-drive
version: 1.1.0
description: "飞书云空间文件管理：管理飞书云空间的文件和文件夹。【触发边界】仅当用户明确要在飞书云空间/上传到飞书/分享文件给同事，或对象已是飞书云空间文件时使用；用户只是要本地文件操作（未点名飞书）则不要用本 skill。功能：上传/下载文件、创建文件夹、复制/移动/删除文件、查看文件元数据、管理文档评论、管理文档权限、订阅用户评论变更事件。"
metadata:
  requires:
    bins: ["lark-cli"]
